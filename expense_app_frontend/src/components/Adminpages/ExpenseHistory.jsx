...
const ExpenseHistory = () => {
  ...
  const fetchData = useCallback(async (appliedFilters = filters) => {
    ...
    try {
      ...
      const regularData = await getGroupedOrders(1, 100, appliedFilters); 
      const regularExpenses = Object.keys(regularData.results).map((date) => {
        const items = regularData.results[date];
        const description = items.map((item) => sanitizeText(item.item_name)).join(", ");
        const amount = items.reduce((sum, item) => sum + item.count * item.price, 0);
      ...
      alert(
        error.response?.data?.error || "Failed to fetch expense history. Please try again."
      );
    } finally {
      setLoading(false);
    }
        ...
        <>
          ...
          <div className="md:hidden space-y-4">
            {currentExpenses.map((exp, i) => (
              <div
                key={exp.id}
                className={`p-4 rounded-lg ${i % 2 === 0 ? "bg-white" : "bg-gray-50"}`}
              >
                <div className="text-sm text-gray-600">
                  <div><strong>S.No:</strong> {(currentPage - 1) * PAGE_SIZE + i + 1}</div>
                  <div><strong>Date:</strong> {formatDate(exp.date)}</div>
                  <div><strong>Description:</strong> {exp.description}</div>
                  <div><strong>Category:</strong> {exp.expense_category}</div>
                  <div><strong>Type:</strong> {exp.expense_type}</div>
                  <div><strong>Amount:</strong> ₹{exp.amount.toFixed(2)}</div>
                  <div><strong>Verified:</strong> {exp.expense_category === "Regular" ? "–" : exp.is_verified ? "Yes" : "No"}</div>
                </div>
              </div>
            ))}
          </div>
